...
class AIRACCycle {
    ...
    public static function fromEffectiveDate(DateTime $date): static 
    {
        ...
        if (floor($serial) !== $serial) throw new InvalidAIRACEffectiveDateException("{$date->format(DateTime::ATOM)} is not a valid effective date");
    ...
    public static function fromCycleCode(string $cycleCode): static 
    {
        ...
        if (count($matches) !== 3) throw new InvalidAIRACCycleCodeException("{$cycleCode} was not of format YYOO");
        ...
        if ((int)$airac->getEffectiveDate()->format('Y') !== $year) throw new InvalidAIRACCycleCodeException("Year {$year} does not have {$ordinalPart} cycles");
        return $airac;
    }
    ...
    {
        if ($serial < 0) throw new InvalidAIRACSerialException("{$serial} is not a valid AIRAC serial");
    }
